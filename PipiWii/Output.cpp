...
void mixTable() {
  ...
  uint8_t i;
     ...
     for(int i=0;i<8;i++) motor[i]=MINCOMMAND;
  ...
  if (f.TRIKOPLAN_mode) {
    ...
    servo[4] = (SERVODIR(4, 128) < 0 ?  conf.servoConf[4].max : conf.servoConf[4].min); // GIMBAL PITCH
    servo[6] = get_middle(6); // aileron roll
    servo[7] = get_middle(7); // aileron roll
  }
