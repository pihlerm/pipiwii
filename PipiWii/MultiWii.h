...
#if defined(AIRSPEED)
  extern int16_t ref_pressure, air_pressure, pressure_diff;
  extern const float airspeed_ratio;
  extern uint16_t Critical_Stall_Speed;
  extern uint16_t airspeed;
  extern boolean AirSpeedCalFlag;
  extern void CalAirSpeedSensor();
  extern void ReadAirSpeed();
#endif

#if defined(TRIKOPLAN)
  extern uint16_t tkp_mix;       // 1 = use TRIKOPLAN special mix; 0 = use TRI mix
  extern float tkp_c_fi;       // [0..kmax]
  extern float tkp_c_pow;       // [0..kmax]
  extern float tkp_fiangle;     // 0..1 angle of propeller
