...
#if defined(AIRSPEED)
  int16_t ref_pressure, air_pressure, pressure_diff;
  const float airspeed_ratio = 9;
  uint16_t Critical_Stall_Speed = 0;
  uint16_t airspeed;
  boolean AirSpeedCalFlag = false;
#endif

#if defined(TRIKOPLAN)
  uint16_t tkp_mix;       // 1 = use TRIKOPLAN special mix; 0 = use TRI mix
  float tkp_c_fi;       // [0..kmax]
  float tkp_c_pow;       // [0..kmax]
  float tkp_fiangle;
  uint16_t tkp_fmotors;        // 1000-2000 front motors speed
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  case 1:
  {
      ...
      uint16_t v = analogRead(V_BATPIN);
      #if VBAT_SMOOTH == 1
        analog.vbat = (v<<6) / conf.vbatscale + VBAT_OFFSET; // result is Vbatt in 0.1V steps
      #else
        vsum += v;
        vsum -= vvec[ind];
        vvec[ind++] = v;
        ind %= VBAT_SMOOTH;
        #if VBAT_SMOOTH == 16
          analog.vbat = (vsum<<2) / conf.vbatscale + VBAT_OFFSET; // result is Vbatt in 0.1V steps
        #elif VBAT_SMOOTH < 16
          analog.vbat = ((vsum<<2) * (16/VBAT_SMOOTH)) / conf.vbatscale + VBAT_OFFSET; // result is Vbatt in 0.1V steps
        #else
          analog.vbat = (((vsum<<2) /VBAT_SMOOTH) * 16) / conf.vbatscale + VBAT_OFFSET; // result is Vbatt in 0.1V steps
        #endif
      #endif
    #endif // VBAT
    break;
