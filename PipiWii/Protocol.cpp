...
#define MSP_AIRSPEED             125   //out message         Send Airspeed
...
#define MSP_AIRSPEED_CALIBRATE   216   //in message          Calibrate the Airspeed Sensor
...
#ifdef DEBUGMSG
  #define DEBUG_MSG_BUFFER_SIZE 256
...
#if defined(AIRSPEED)     
   case MSP_AIRSPEED_CALIBRATE:
     if(!f.ARMED) CalAirSpeedSensor();
     headSerialReply(0);
     break;
  
   case MSP_AIRSPEED:
       headSerialReply(2);
       serialize16(airspeed);
       break;
#endif
...
void debugmsg_append_char(const char c) {
  debug_buf[head_debug++] = c;
  if (head_debug == DEBUG_MSG_BUFFER_SIZE) {
     head_debug = 0;
  }
}
