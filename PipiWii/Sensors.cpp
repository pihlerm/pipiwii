...
// ************************************************************************************************************
#if defined(AIRSPEED)
void CalAirSpeedSensor()
{
    delay(1000);
        
    ref_pressure = analogRead(AIRSPEEDPIN);
    
    for (uint8_t i=1;i<=100;i++)
    {
     ref_pressure = (analogRead(AIRSPEEDPIN))*0.1 + ref_pressure*0.9;
      
      delay(20);
    }
    
    AirSpeedCalFlag = true;
}

void ReadAirSpeed()
{
      air_pressure = analogRead(AIRSPEEDPIN) * 0.05 + air_pressure * 0.95;
      if (air_pressure >= ref_pressure)
          pressure_diff = air_pressure - ref_pressure;
      else
          pressure_diff = 0.0;
            
      airspeed = sqrt(pressure_diff * airspeed_ratio) * 100;
}
#endif
...
#define SONAR_ALT_BUF_SIZE 5
// which element (sorted by size) to use as median
#define SONAR_ALT_BUF_MEDIAN 2  
static int32_t  sonarAlt_buffer[SONAR_ALT_BUF_SIZE];
static uint8_t sonarAlt_buffer_curr=0;
...
void Sonar_init()
{
  ...
  for(uint8_t j=0;j<SONAR_ALT_BUF_SIZE;++j) {
	sonarAlt_buffer[j] = -1;
  }
...
void Sonar_update()
{
  sonarAlt_buffer[sonarAlt_buffer_curr] = tempSonarAlt;
  ++sonarAlt_buffer_curr;
  if(sonarAlt_buffer_curr >= SONAR_ALT_BUF_SIZE) sonarAlt_buffer_curr=0;

  // get median element
  uint8_t i,j;
  int32_t median_alt_idx = -1;
  int32_t median_alt = -1;
  int32_t temp_median_alt_idx;
  int32_t temp_median_alt;
  
  for(i=0;i<=SONAR_ALT_BUF_MEDIAN;++i) {
	  temp_median_alt_idx = -1;
	  temp_median_alt = SONAR_GENERIC_MAX_RANGE+1;
	  for(j=0;j<SONAR_ALT_BUF_SIZE;++j) {
		if( (sonarAlt_buffer[j] < temp_median_alt) &&
		    ((sonarAlt_buffer[j] > median_alt) || ((sonarAlt_buffer[j] == median_alt) && (j>median_alt_idx)))
		  )
		  {
			temp_median_alt_idx = j;
			temp_median_alt = sonarAlt_buffer[j];
			if(temp_median_alt == median_alt) break;
		  }
	  }
	median_alt_idx = temp_median_alt_idx;
	median_alt = temp_median_alt;
  }
